...
const ReportHistoryCard: React.FC<ReportHistoryCardProps> = ({ report }) => {
  const { aiAnalysis, complaint, createdAt, id: reportId } = report;
  ...
  const handleCardPress = () => {
    if (aiAnalysis) {
      const navigationParams = {
        testData: JSON.stringify(aiAnalysis),
      };
      router.push({
        pathname: "/main/chat",
        params: navigationParams as any,
      });
    } else {
      console.log(
        `[ReportHistoryCard] aiAnalysis tidak ditemukan untuk laporan ID: ${reportId}. Navigasi hanya dengan reportId.`
      );
      router.push({
        pathname: "/main/chat",
        params: { reportId: reportId } as any,
      });
    }
  };
